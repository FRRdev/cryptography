...
# Length of the chunk block
CHUNK_LENGTH = 8

# asserting for correct chunk length
assert not CHUNK_LENGTH % 8, 'chunk length must be mod by 8'

# calculate for check bits in hamming code
CHECK_BITS = [i for i in range(1, CHUNK_LENGTH + 1) if not i & (i - 1)]


def chars_to_bin(chars: list) -> str:
    assert not len(chars) * 8 % CHUNK_LENGTH, \
        'The length of the encoded data must be a multiple of the length of the encoding block'
    return ''.join([bin(ord(c))[2:].zfill(8) for c in chars])


def chunk_iterator(text_bin: list, chunk_size: int = CHUNK_LENGTH) -> Generator:
    for i in range(len(text_bin)):
        if not i % chunk_size:
            yield text_bin[i:i + chunk_size]


def get_check_bits_data(value_bin: list) -> dict:
...
def set_empty_check_bits(value_bin: list) -> List[str]:
    for bit in CHECK_BITS:
        value_bin = value_bin[:bit - 1] + '0' + value_bin[bit - 1:]
    return value_bin


def set_check_bits(value_bin: list) -> list:
...
def get_check_bits(value_bin: list) -> dict:
    check_bits = {}
    for index, value in enumerate(value_bin, 1):
...
def exclude_check_bits(value_bin: list) -> str:
    clean_value_bin = ''
...
def set_errors(encoded_data: list) -> str:
    result = ''
    for chunk in chunk_iterator(encoded_data, CHUNK_LENGTH + len(CHECK_BITS)):
...
def check_and_fix_error(encoded_chunk: list) -> str:
...
def get_diff_index_list(value_bin1: list, value_bin2: list) -> list:
    _diff_index_list = []
    for index, char_bin_items in enumerate(zip(list(value_bin1), list(value_bin2)), 1):
        if char_bin_items[0] != char_bin_items[1]:
            _diff_index_list.append(index)
    return _diff_index_list


def encode(source_data: list) -> str:
    text_bin = chars_to_bin(source_data)
    result = ''
...
def decode(encoded_data: list, fix_errors: bool = True) -> str:
    decoded_value = ''
    fixed_encoded_list = []
    for encoded_chunk in chunk_iterator(encoded_data, CHUNK_LENGTH + len(CHECK_BITS)):
        if fix_errors:
...
source = "testmessage"
print('Length of decode chunk: {0}'.format(CHUNK_LENGTH))
print('Control bits: {0}'.format(CHECK_BITS))
encoded = encode(source)
print('Encoded data: {0}'.format(encoded))
decoded = decode(encoded)
print('Result of decoding: {0}'.format(decoded))
encoded_with_error = set_errors(encoded)
print('Set errors for encoded data: {0}'.format(encoded_with_error))
diff_index_list = get_diff_index_list(encoded, encoded_with_error)
print('Errors in bits: {0}'.format(diff_index_list))
decoded = decode(encoded_with_error, fix_errors=False)
print('Result of decoded data with errors in channel: {0}'.format(decoded))
decoded = decode(encoded_with_error)
print('Result of decoded data with errors in channel with error correcting {0}'.format(decoded))